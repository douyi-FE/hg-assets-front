<template>
  <a-drawer :title="formState.id ? '编辑模板' : '新增模板'" width="100%" :open="isOpen" :get-container="false"
    :style="{ position: 'absolute' }" @close="isOpen = false">
    <template #extra>
    ...
    <a-row :gutter="20" style="height: 100%">
      ...
      <a-col :span="6">
        <a-card title="模板信息">
          <a-form ref="formRef" :model="formState" :label-col="{ style: { width: '100px' } }"
            :wrapper-col="{ span: 20 }">
        ...
        <a-card title="历史版本" style="margin-top: 10px; max-height: 600px; overflow-y: auto">
          <a-table :columns="versionColumns" :data-source="versionList" :rowClassName="getRowClassName">
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.dataIndex === 'action'">
                ...
                <a-popconfirm title="确认删除吗?" ok-text="确认" cancel-text="取消" @confirm="handleDeleteVersion(record)">
