<template>
  <div class="device-table-wrapper">
    <a-table :columns="deviceColumns" :data-source="dataSource" rowKey="_id" :pagination="false" size="small">
      <template #title>
        <div class="table-header-wrapper">
          <a-button size="small" type="primary" :disabled="props.isDone" @click="
            open = true;
          currentDevice = {
            id: undefined,
            name: '',
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'operation'">
          <a-popconfirm title="删除会导致设备数据丢失，是否确认删除?" ok-text="是" cancel-text="否" @confirm="deleteDeviceHandler(record)">
            <a-button type="link" :disabled="props.isDone">删除</a-button>
          </a-popconfirm>
          <a-button type="link" :disabled="props.isDone" @click="
            open = true;
          currentDevice = {
            id: record._id,
            name: record.name,
...
const props = withDefaults(defineProps<{ projectCode: string, isDone: boolean }>(), {
  projectCode: '',
  isDone: false,
});
...
const fetchDeviceData = function () {
  getDeviceList(props.projectCode).then((res) => {
