    ...
    <a-table :columns="engineerColumns" :data-source="dataSource" :pagination="false">
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'operation'">
          ...
          " :disabled="props.isDone">编辑工程</a-button>
          <a-popconfirm title="删除会导致工程数据丢失，是否确认删除?" ok-text="是" cancel-text="否"
...
const props = withDefaults(defineProps<{ projectCode: string; deviceCode: string; isDone: boolean }>(), {
  projectCode: '',
  deviceCode: '',
  isDone: false,
});
...
const fetchEngineerList = async () => {
  const res = await getEngineerList(props.projectCode, props.deviceCode);
