...
<script lang="ts" setup>
  ...
  const fetchProjectData = async function () {
    const result: any[] = await getApplicationByName('工程咨询服务合同登记表').then(
      // 替换【咨询合同台账】
      async (res) => {
        return await getApplicationData({ templateId: res.templateId }).then((res) => {
          ...
          const hasEngineerData = tableData.filter(
            (item: any) =>
              item['是否含工程量计算书'] === '是' && item['工程量计算是否完成'] === '否',
          );
          if (hasEngineerData.length === 0) {
            return [];
          }
          return hasEngineerData;
        });
      },
    );
