...
// 依据ids获取excel的template
export async function getExcelTemplateByIds(ids: string[]) {
  return request<any>({
    url: '/api/template/list/ids',
    method: 'GET',
    params: { ids },
  });
}
...
export async function saveExcelTemplate(data: any, options?: RequestOptions) {
...
export async function updateExcelTemplate(data: any, options?: RequestOptions) {
...
export async function publishExcelTemplate(id: string) {
  return request<any>('/api/template/excel/publish', {
    method: 'POST',
    params: { id },
  });
}
